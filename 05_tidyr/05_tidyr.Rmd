...
This is all we need to know for this workshop, but if you are interested in the details make sure to check out [this article](https://www.jstatsoft.org/article/view/v059i10).
...
Here are some examples how easy it is to work with `table1`:
...
Note that because these two columns contain names that would be a syntax error (names must not start with a digit), we have to enclose them with backticks. Furthermore, the type of the `year` column is character, because this is the default behavior for columns created with the `names_to` argument. If we wanted an integer column, we could use the `names_transform` argument.
