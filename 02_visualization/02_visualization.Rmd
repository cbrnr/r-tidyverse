...
## Introduction
Data needs to be imported, tidied, and transformed before any visualization can be created. We will discuss these steps, which are necessary for almost all real-world data sets, in the following chapters. However, because we will use tidy example data sets that are available in R, we will focus on creating visualizations first.

We will use the [ggplot2](https://ggplot2.tidyverse.org/) package, which is based on a layered "grammar of graphics" described in [this article](http://vita.had.co.nz/papers/layered-grammar.pdf). In this short chapter, we will mention only the most basic commands that enable you to quickly create simple (but beautiful) visualizations.
...
Note that you could also use `library(tidyverse)` instead, because `ggplot2` is part of the `tidyverse` package.
...
Here's the first question we want to answer from this data set: Do cars with big engines consume more or less fuel than cars with small engines? We can use the variables `displ` (engine size in liters) and `hwy` (fuel efficiency on the highway in miles per gallon) to try to address the question.
...
This scatterplot shows a clear negative relation, meaning that larger engines tend to have lower fuel efficiency. 

All ggplot2 plots start with calling the `ggplot()` function, to which we pass a data frame that contains the data to be plotted (using the `data` argument). This function call on its own just produces an empty plot, because we haven't specified any details about our plot.
...
There are other aesthetics that we could map data to. For example, we could use the `color` aesthetic to visualize a third variable in the data, for example `class`, which lists the type (or class) of car:
...
If you ever want to manually set an aesthetic to a fixed value (not depending on data values), such as plotting all points in blue, you need to specify this outside of the `aes()` function:
...
## Composing plots
Facets create subplots based on subsets of the data. If instead you want to combine multiple *independent* plots, I recommend the [patchwork](https://patchwork.data-imaginist.com/) package. Using operators such as `+`, `|`, and `/`, it is intuitive to create custom arrangements of existing `ggplot` objects (which we need to store in objects for easier access), for example:
...
Please refer to the patchwork documentation and website for more details. 
...
## Coordinate systems
Most ggplot2 plots use the Cartesian coordinate system spanned by two axes *x* and *y*. However, it is possible to change the coordinate system with so-called *coord* functions. We won't go into any detail here, mostly because knowing how to tewak coordinate systems is quite an advanced topic that is not relevant for many real-world plots. However, there is one handy function that you should remember: `coord_flip()` flips the *x* and *y* axes. You can simply add it like an additional layer to an existing plot:
