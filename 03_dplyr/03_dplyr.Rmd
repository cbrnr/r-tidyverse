...
## Introduction
### Prerequisites
In this chapter, we will introduce the [dplyr](https://dplyr.tidyverse.org/) package, another core member of the Tidyverse. Because we have not discussed importing our own data sets yet, we will showcase data manipulation with dplyr with the `nycflights13::flights` data set. We will also apply our knowledge from the previous chapter on ggplot2 by creating various visualizations of this data set. 

In this chapter, we will need to activate the following packages:
...
After activating `dplyr`, a message informs us that several functions from base R have been masked, namely `filter()`, `lag()`, `intersect()`, `setdiff()`, `setequal()`, and `union()`. This means that calling `filter()` after `dplyr` has been loaded doesn't refer to the base R function from the `stats` package anymore. If you still want to use that function, you need to use its full name `stats::filter()`.

### The nycflights13 package
The `nycflights13` package contains five data sets on all 336,776 flights that departed New York City in 2013. We will be using the `flights` data frame in this chapter. Before we start, take a look at the documentation `?flights` to find out what each column (variable) represents.
...
## Basic verbs
The dplyr package offers six functions that form the verbs of a data manipulation language. With these six functions, we can solve almost any data manipulation problem. In particular, these six functions perform the following tasks:
...
- `summarize()` aggregates many data points to a single value.
- `group_by()` changes the operations to work on grouped data subsets instead of the entire data set.

We will discuss each function using example operations on the `nycflights::flights` data set. All functions share some common consistent behavior, because they take a data frame as their first argument and always return a new data frame as their result.
...
The result is a new data frame consisting of 842 rows, which represent all flights on that date. Notice that we can specify column names directly without having to prepend `flights$`, because `filter()` is smart enough to know that we are referring to column names inside the `flights` data frame.

R knows the usual comparison operators:
...
When we pass two or more comparison arguments, `filter()` combines them with "and" (as in the previous example). If we need other logical operations, we can combine comparisons with these logical operators:
...
### Exercises
See https://r4ds.had.co.nz/transform.html#exercises-8.

## Arrange rows
Instead of filtering, it is often useful to arrange rows in a specific order. Similar to `filter()`, the `arrange()` function takes a data frame as its first argument, and one or more additional arguments specify how sorting should be performed. Here's how to sort the data by scheduled departure time as well as year, month, and day:

```{r}
arrange(flights, year, month, day, sched_dep_time)
```

By default, `arrange()` uses ascending order, but we can use the `desc()` function to use descending order:
...
### Exercises
See https://r4ds.had.co.nz/transform.html#exercises-9.

## Select variables
Often, only a subset of columns in the original data is needed for a particular analysis task. The `select()` function picks only those columns you specify as arguments. Note that ranges can be specified with the `:` operator, and negative values indicate the complement of a specific selection.
...
- `matches("(.)\\1")` finds names that match the regular expression (regex) `(.)\\1` (repeated characters in this particular regex). Regular expressions are a powerful tool for matching almost any kind of pattern, but we do not have time to cover them in this workshop.
- `num_range("x", 1:3)` finds names `x1`, `x2`, and `x3`.

Here are two examples with the `flights` data set:
...
```{r}
select(flights, time_hour, air_time, everything())  # move time_hour and air_time columns to the start
```

### Exercises
See https://r4ds.had.co.nz/transform.html#exercises-10.

## Create new variables
The `mutate()` function adds new columns based on values from existing columns. Because new columns are always added to the end, we will first create a subset of the `flights` data frame with fewer columns to see the effect of `mutate()`.
...
Note that we can even refer to columns that we have only just created (the `hours` column in the previous example is immediately used in creating the `speed` column).

Whereas `mutate()` always keeps all columns, `transmute()` keeps only the new ones.

### Exercises
See https://r4ds.had.co.nz/transform.html#exercises-11.
...
The `group_by` function partitions the data set into groups. Applying `summarize()` to grouped data will automatically compute the summary for each group. In the following example, we compute the average delay for each day:
...
### The pipe operator
One of the key properties of Tidyverse functions is their support for the pipe operator `%>%`, which is defined in the `magrittr` package. However, we rarely need to activate this package explicitly, because `dplyr` takes care of that under the hood.

The idea behind piping is simple: instead of having to store an intermediate value in a variable, the pipe operator forwards it as the first argument to the next function. This means that instead of writing `f(x)`, we can express the same command as `x %>% f()`. For example, `mean(x)` could also be written as `x %>% mean()`. If we wanted to feed the result of `f(x)` into another function `g()`, instead of writing `g(f(x))` we could use a pipe `x %>% f() %>% g()`. That way, computations that depend on previous calculation can be written in a very intuitive way.

You might be wondering why this syntactic sugar is so popular in the Tidyverse (after all, it doesn't add any new functionality). One of the main reasons is that it allows us to write code that corresponds to the flow of operations. Consider the previous example `g(f(x))`, where we take `x`, compute `f()`, and feed its result into `g()`. It is much easier to see what's going on when we write this expression as `x %>% f() %>% g()`.
...
Note that R has gained a native pipe operator as of version 4.1. It is written as `|>` and we can use it instead of `%>%` in almost all cases.

### Counts
R comes with a plethora of useful summary functions. Computing counts in grouped data is always a good idea, because it can provide important insights. The `n()` function can be used to determine group sizes, but note that missing values are included in the counts. You can use `sum(!is.na(x))` to count the number of non-missing data points in a (grouped) variable `x`:
...
Let's take a look at the average arrival delay of each individual airplane (as identified by its unique tail number). First, we drop all rows where either departure delay or arrival delay values are missing. Then we group by `tailnum` and summarize these groups by computing the mean arrival delay. Finally, because the result of this pipeline is a data frame, we can directly pipe that to `ggplot()` to create a visualization of the average delay distribution.
...
This plot shows that most airplanes are delayed by only a few minutes, but there are some airplanes with an average arrival delay of around five hours! In this context, it would be interesting to know the number of data points used for each average &ndash; it is likely that the airplanes with extremely high average delays are really based on only a few flights. We can use the `n()` function to find out and create a scatterplot of average delay versus number of data points for each delay:
...
- Measures of position: `first()`, `last()`, `nth()`.
- Counts: `n()`, `n_distinct()`, `count()`.
...
Which destinations have the hightest number of carriers?
...
### Ungrouping
Sometimes it is necessary to remove grouping and return to the full data set. This can be achieved with the `ungroup()` function.

### Exercises
See https://r4ds.had.co.nz/transform.html#exercises-12.
