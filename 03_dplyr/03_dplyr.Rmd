...
The `nycflights13` package contains five datasets on all 336,776 flights that departed New York City in 2013. We will be using the `flights` data frame in this chapter. Before we start, take a look at the documentation `?flights` to find out what each column (variable) represents.
...
- `group_by()` changes the operations to work on grouped data subsets instead of the entire dataset.

We will discuss each function using example operations on the `nycflights::flights` dataset. All functions share some common consistent behavior, because they take a data frame as their first argument and always return a new data frame as their result.
